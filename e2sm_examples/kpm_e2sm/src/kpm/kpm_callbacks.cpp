...
void callback_kpm_subscription_request(E2AP_PDU_t *sub_req_pdu)
{
	...
	fprintf(stderr, "Calling callback_kpm_subscription_request (modified)\n");
	...
	*/
	...
	if (triggerDef->buf)
	{
		...
		fprintf(stderr, "(modified) triggerdef buf is present with length %lu", triggerDef->size);
		// assert(1==0);
...
void callback_kpm_control(E2AP_PDU_t *control_pdu)
{
	...
	for (int idx = 0; idx < ricControlRequest->protocolIEs.list.count; idx++)
	{
		...
		case RICcontrolRequest_IEs__value_PR_RICrequestID:
		{
			...
			case 24:
			{
				fprintf(stderr, "Trilaterarion xApp message\n");
				break;
			}
			}
			break;
